...
10. **Qualitative Code Review.** For the 53% of standards that linters cannot check (meaningful names, substantive descriptions, business rule tests), you must actively self-reflect before saving files. At the end of major layer boundaries (Staging, Integration, Marts), you must invoke `python -m scripts.reviewer --select <model> --agent`. Read the generated checklist template from `tmp/review_<model>.md`, fill it out noting PASS/FAIL and rationale, and optionally request a qualitative peer review from the user if you have questions. `reference/dbt_project_standards.json` is available if you need to double-check rule definitions.
