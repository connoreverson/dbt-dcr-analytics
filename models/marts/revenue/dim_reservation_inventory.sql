...
inventory_enriched as (
    select
        ...
        concat_ws(', ', 
            case when int_customer_assets.pet_policy = 'allowed' then 'Pets Allowed' else null end,
            case when int_customer_assets.utility_hookup = 'full' then 'Full Hookups' else null end
        ) as amenities,

        -- enrichments
        case
            when int_customer_assets.asset_type in ('campsite', 'tent', 'rv', 'rv_site', 'tent_site') then 'Camping'
            when int_customer_assets.asset_type in ('cabin', 'yurt', 'lodge') then 'Lodging'
            when int_customer_assets.asset_type in ('shelter', 'day-use', 'picnic_area') then 'Day Use'
            else 'Other'
        end as asset_type_group,

        case
            when int_customer_assets.max_occupancy <= 4 then 'Small (1-4)'
            when int_customer_assets.max_occupancy between 5 and 8 then 'Medium (5-8)'
