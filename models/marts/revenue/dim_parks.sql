...
parks_enriched as (
    select
        ...
        cast(int_parks.address1_stateorprovince as varchar) as address1_stateorprovince,
        cast(int_parks.address1_postalcode as varchar) as address1_postalcode,
        cast(int_parks.address1_latitude as decimal(10,6)) as address1_latitude,
        cast(int_parks.address1_longitude as decimal(10,6)) as address1_longitude,
        
        case 
            when cast(int_parks.total_acres as float) < 500 then 'Small (< 500 acres)'
            when cast(int_parks.total_acres as float) between 500 and 5000 then 'Medium (500 - 5000 acres)'
            when cast(int_parks.total_acres as float) > 5000 then 'Large (> 5000 acres)'
            else 'Unknown'
