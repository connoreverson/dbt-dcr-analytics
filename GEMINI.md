...
24. **Trust Tool Selectors over Scripted Loops.** If a python script or dbt command supports `--select` for a directory, pass the directory path directly (e.g., `--select models/integration`). Do not write elaborate PowerShell `foreach` loops to invoke the script multiple times per file.
