...
10. **PowerShell Virtual Environment Execution.** This is a hard constraint — do not experiment with alternatives. When executing commands within a Python virtual environment via PowerShell, you **must** use dot-sourcing: `. .\.venv\Scripts\Activate.ps1; dbt ...`. Do not use the call operator (`&`), `$env:PATH` manipulation, or direct `.venv\Scripts\dbt.exe` invocation — all of these have failed in practice. When initializing a dbt project, always use `dbt init <project_name> --skip-profile-setup` — the `--no-interactive` flag does not exist and `--skip-profile-setup` is what prevents the interactive prompt from hanging. If a command fails, re-read this rule before trying alternatives.
