...
def run_summarize(input_dir: str) -> int:
    """Summarize review YAML files in input_dir, writing to a sibling markdown file.

    Returns 0 on success, 1 on error.
    """
    output_file = os.path.join(os.path.dirname(os.path.abspath(input_dir)), "failed_reviews_summary.md")
    try:
        os.makedirs(os.path.dirname(os.path.abspath(output_file)), exist_ok=True)
        summarize_reviews(input_dir, output_file)
        print(f"Summary generated successfully at: {output_file}")
        return 0
    except Exception as e:
