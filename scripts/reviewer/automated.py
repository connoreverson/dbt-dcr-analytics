...
# ---------------------------------------------------------------------------
# New entry points for scripts.reviewer package
# ---------------------------------------------------------------------------
...
def run_model_review(selector: str, agent: bool = False) -> tuple[int, dict]:
    ...
    """
    results = run_checks(selector, quiet=agent)
...
def run_branch_review(branch: str, agent: bool = False) -> int:
    ...
    # Filter to SQL model files and extract model names
    model_names = []
...
def _print_branch_summary_markdown(branch: str, results: list[dict]) -> None:
    ...
    print("| Model | Passed | Failed | Warnings |")
    print("|---|---|---|---|")
    for r in results:
        model = r["model"]
        ...
        for r in failures:
            ...
            for detail in r.get("details", []):
                if detail.status == "FAIL":
                    msgs = "; ".join(detail.messages[:2]) if detail.messages else ""
                    print(f"- \u2717 {detail.name}: {msgs}")
